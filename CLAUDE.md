# CLAUDE.md — Story Engine (Pocket FM Hackathon)

> **This is a map, not a manual.** Entry file for coding agents — kept short on purpose (≤200 lines).
> Detailed rules live in on-demand topic docs (see *Topic Docs*). Global `~/.claude/CLAUDE.md` rules
> still apply (git-commit permission, never-guess, thinking-partner) and are not repeated here.
> `AGENTS.md` is a mirror of this file for non-Claude agents — **keep the two in sync.**
